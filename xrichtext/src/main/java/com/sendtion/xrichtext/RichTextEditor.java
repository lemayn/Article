...
//        allLayout.setPadding(50, 15, 50, 15);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
        //FiDo界面间距调整为10
        allLayout.setPadding(10, 10, 10, 10);//设置间距，防止生成图片时文字太靠边，不能用margin，否则有黑边
...
//        EditText firstEdit = createEditText("请输入内容", dip2px(context, EDIT_PADDING));
        EditText firstEdit = createEditText("", dip2px(context, 5));
...
//                editStr2 = " ";
    ...
    public void addImageViewAtIndex(final int index, String imagePath) {
        ...
        int imageHeight = 500;
        Log.i("FiDo", "addImageViewAtIndex: "+"图片宽度为："+bmp.getWidth() + "图片高度为："+bmp.getHeight());
        if (bmp != null) {
